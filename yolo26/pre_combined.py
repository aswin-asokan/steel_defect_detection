#!/usr/bin/env python3
"""
YOLO -> MobileSAM real-time defect pipeline with preprocessing.
- Preprocessing: illumination correction, CLAHE, blur, gamma, optional specular inpaint
- YOLO runs on preprocessed frames (3-channel)
- SAM can run on preprocessed or original frames (configurable)
- CSV + JSON logging, save raw/overlay/mask images
"""
import os
import time
import csv
...
class Config:
    ...
    # --- PREPROCESSING ---
    PREPROCESS = True                # master switch to enable preprocessing
    PREPROCESS_BG_KERNEL = 51        # must be odd; used for background blur in illumination estimation
    PREPROCESS_CLAHE_CLIP = 2.0
    PREPROCESS_CLAHE_TILE = (8,8)
    PREPROCESS_BLUR = (3,3)          # small blur to mimic dataset texture
    PREPROCESS_GAMMA = 1.0           # 1.0 = no gamma change
    PREPROCESS_SPECULAR_THRESHOLD = 240  # bright pixel threshold to consider specular highlights (0 to disable)
    PREPROCESS_INPAINT_RADIUS = 3
    SAM_USE_PREPROCESS = True        # run SAM on preprocessed frame? (True recommended if SAM fine-tuned on preprocessed patches)
...
# load SAM adapter / checkpoint
if not os.path.exists(config.SAM_MODEL_PATH):
    raise FileNotFoundError(f"SAM model weights not found: {config.SAM_MODEL_PATH}")

checkpoint = torch.load(
    config.SAM_MODEL_PATH,
    map_location=config.DEVICE,
    weights_only=False   # IMPORTANT: restore old behavior
)
...
def gamma_correction(img, gamma):
    ...
    table = ((np.arange(256) / 255.0) ** invGamma * 255.0).astype("uint8")
    return cv2.LUT(img, table)

def preprocess_frame(frame):
    """
    Convert BGR frame -> preprocessed BGR image that looks like dataset patches.
    Steps:
      - grayscale
      - illumination correction (background division)
      - CLAHE
      - small blur
      - gamma correction
      - optional specular highlight removal (inpaint)
      - normalize to 0..255 and convert back to 3-channel BGR
    """
    ...
    # illumination correction by background division
    # choose kernel reasonably (<= min(w,h)-1 and odd)
    max_k = max(3, min(w, h) - 1)
    bg_k = min(config.PREPROCESS_BG_KERNEL, max_k)
    bg_k = _ensure_odd(max(3, bg_k))
    background = cv2.GaussianBlur(gray, (bg_k, bg_k), 0)
    # avoid division by zero; scale back to 0-255
    illum = cv2.divide(gray, background, scale=255.0)
    illum = np.clip(illum, 0, 255).astype(np.uint8)

    # CLAHE
    clahe = cv2.createCLAHE(clipLimit=config.PREPROCESS_CLAHE_CLIP, tileGridSize=config.PREPROCESS_CLAHE_TILE)
    cl = clahe.apply(illum)

    # small gaussian blur
    if config.PREPROCESS_BLUR is not None and config.PREPROCESS_BLUR[0] > 0:
        cl = cv2.GaussianBlur(cl, config.PREPROCESS_BLUR, 0)

    # gamma correction
    if config.PREPROCESS_GAMMA and abs(config.PREPROCESS_GAMMA - 1.0) > 1e-6:
        cl = gamma_correction(cl, config.PREPROCESS_GAMMA)

    # optional specular highlight removal: detect bright pixels and inpaint
    if 0 < config.PREPROCESS_SPECULAR_THRESHOLD < 255:
        bright_mask = (cl >= config.PREPROCESS_SPECULAR_THRESHOLD).astype(np.uint8) * 255
        if np.any(bright_mask):
            k = 3
            bright_mask = cv2.dilate(bright_mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k,k)), iterations=1)
            try:
                cl = cv2.inpaint(cl, bright_mask, config.PREPROCESS_INPAINT_RADIUS, cv2.INPAINT_TELEA)
            except Exception:
                cl[bright_mask > 0] = int(np.median(cl))

    # normalize to 0-255
    cl = cv2.normalize(cl, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)

    # convert to 3-channel BGR for YOLO
    pre_bgr = cv2.cvtColor(cl, cv2.COLOR_GRAY2BGR)
    return pre_bgr
...
def main():
    ...
    try:
        while True:
            ...
            results = yolo.predict(pre_frame, conf=config.YOLO_CONF, imgsz=512, verbose=False)
            res0 = results[0]
            ...
            if hasattr(res0, "boxes") and len(res0.boxes) > 0:
                try:
                    ...
                    conf_tensor = res0.boxes.conf
                    cls_tensor = res0.boxes.cls
            ...
            # annotated image from YOLO plot (note: plot uses the image that was passed to predict -> pre_frame)
            try:
                annotated = res0.plot()
            except Exception:
                annotated = pre_frame.copy()
            ...
            if defect_detected:
                ...
                # 2) Run SAM on union ROI (or full image); decide which image to pass to SAM
                ...
                sam_input_image_for_roi = pre_frame if config.SAM_USE_PREPROCESS else frame
                if config.USE_ROI and union is not None:
                    mask_full, sam_conf = run_mobilesam_on_roi(sam_input_image_for_roi, union, threshold=config.SAM_THRESHOLD)
                else:
                    mask_full, sam_conf = run_mobilesam_on_roi(sam_input_image_for_roi, (0,0,w,h), threshold=config.SAM_THRESHOLD)

                # 3) Build overlay: mask + contours + YOLO boxes+labels (overlay drawn on original frame so saved overlays show original texture)
                ...
                overlay = frame.copy()
                # red mask overlay (translucent effect)
                red_mask = overlay.copy()
                red_mask[mask_full > 0] = (0, 0, 255)
