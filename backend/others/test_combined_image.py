#!/usr/bin/env python3
"""Run YOLO26 + MobileSAM combined detection on a single image for testing."""
...
DEFAULT_OUTPUT_DIR = BASE_DIR / "test_outputs" / "combined"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="YOLO26 + MobileSAM image test")
    ...
    parser.add_argument(
        "--use-preprocess",
        action="store_true",
        help="Apply backend preprocessing before YOLO inference",
    )
...
def main() -> None:
    ...
    validation = service._ensure_models_for_mode(service.MODE_YOLO26_MOBILE_SAM)
    if not validation["ok"]:
        raise RuntimeError(validation["message"])

    current = service.config_store.get()
    cfg = service.RuntimeConfig(
        mode=service.MODE_YOLO26_MOBILE_SAM,
        use_preprocess=bool(args.use_preprocess),
        sam_use_preprocessed_input=False,
        yolo_conf=float(args.conf),
    ...
    overlay_path = output_dir / f"{image_path.stem}_combined_overlay.jpg"
    mask_path = output_dir / f"{image_path.stem}_combined_mask.png"
    preprocessed_path = output_dir / f"{image_path.stem}_combined_preprocessed.jpg"
    summary_path = output_dir / f"{image_path.stem}_combined_result.json"
    ...
    if result.get("preprocessed_frame") is not None:
        cv2.imwrite(str(preprocessed_path), result["preprocessed_frame"])
    ...
    summary = {
        ...
        "preprocessed_image": str(preprocessed_path) if result.get("preprocessed_frame") is not None else None,
